        ...
        if bid_entry.size < offer_entry.size:
            ...
            self.bid_order_id = \
                order_manager.send_new_order(offer_entry.venue, symbol,
                    BID, offer_entry.price - 0.001, smaller_qty)
            logger.info(
...
def close_unbalanced_cross(bigger, smaller):
    ...
    # partially filled so the new qty should be bigger.filled_qty - smaller.filled_qty
...
def kill_cross():
  ...
  """
    ...
    if bid.filled_qty == ask.filled_qty:
        logger.info('In kill_cross BID and ASK qty filled are same - so cancel if alive (%s, %s)'
                     % (bid.id, ask.id))
        ...
        if bid_alive == False and ask_alive == False:
            logger.warning('Cross seems dead - finding new cross')
    ...
    # or rescue order has died, give up on it!
      ...
      # assert (not rescue_expired), "Rescue expired: %s" % order
            ...
            for a in sorted_offers:
                logger.debug('BID: %s', a)
            for b in sorted_bids:
                logger.debug('ASK: %s', b)
            sys.stdout.flush()
