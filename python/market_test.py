...
logger = create_logger("market_test", 
...
def test_reject_new(symbol='EUR/USD'):
    ...
    if bid_in_market == False:
        ...
        bid_id = order_manager.send_new_order(best_bid.venue, symbol, BID, best_bid.price-0.005, 999999999999)
        t1 = time.time()
...
def test_reject_cancel_replace(symbol='EUR/USD'):
    ...
    if best_bid.price == order_constants.NO_BID or best_ask.price == order_constants.NO_ASK:
    ...
    if bid_in_market == False:
        t0 = time.time()
        logger.info("Sending new")
        bid_id = order_manager.send_new_order(best_bid.venue, symbol, BID, best_bid.price-0.005, 875000)
        t1 = time.time()
        logger.info("Finished new, sending replace")
        replace_id = order_manager.send_cancel_replace(bid_id, best_bid.price-0.003, 9999999999999)
        t2 = time.time()
...
def test_cancel_replace(symbol='EUR/USD'):
    ...
    if bid_in_market == False:
        ...
        t1 = time.time()
        logger.info("Finished new, sending replace")
        replace_id = order_manager.send_cancel_replace(bid_id, best_bid.price-0.003, 250000)
        t2 = time.time()
        logger.info("Finished replace")
        logger.info("*********")
        logger.info("Send new took: %f" % (t1-t0))
...
def test_hit_ask_single(symbol='EUR/USD', qty=1000000):
    ...
    if ask_in_market == False:
        t0 = time.time()
        logger.info("Sending new bid at current bestk ask: %f", best_ask.price)
        bid_id = order_manager.send_new_order(best_ask.venue, symbol, BID, best_ask.price, qty)
        t1 = time.time()
        logger.info("*********")
        logger.info("Send new took: %f" % (t1-t0))
        logger.info("*********")
...
def test_hit_bid_single(symbol='EUR/USD', qty=1000000):
    ...
    if bid_in_market == False:
        t0 = time.time()
        logger.info("Sending new ask at current bestk ask: %f", best_bid.price)
        bid_id = order_manager.send_new_order(best_bid.venue, symbol, ASK, best_bid.price, qty)
        t1 = time.time()
        logger.info("*********")
        logger.info("Send new took: %f" % (t1-t0))
        logger.info("*********")
...
def test_cancel_replace_with_partials(symbol='EUR/USD'):
    ...
    if bid_in_market == False:
        ...
        t1 = time.time()
        logger.info("Finished new, sending cancel")
        order_manager.send_cancel_replace(bid_id, best_bid.price, 5555000)
        t2 = time.time()
        logger.info("Finished cancel")
        logger.info("*********")
        logger.info("Send new took: %f" % (t1-t0))
