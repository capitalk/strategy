...
class OrderManager:
    ...
    # print "pending id accepted", order_id,  pending_id
        ...
        logger.info('Renaming order_id %s to %s',
                    self.get_order(order_id).id, pending_id)
        self.DBG_ORDER_MAP()
    ...
    # only used for cancel and cancel/replace
        ...
        original_order_id = uuid.UUID(bytes=er.orig_cl_order_id)
        ...
        logger.info(
            'Exec Report: venue_id = %d, order_id = %s, orig_id = %s, status = %s, exec_type = %s, transaction_type= %s, side = %s, price = %f, qty = %d, filled_qty = %d, unfilled_qty = %d, avg_price = %f'
                ,
            er.venue_id,
            order_id,
            original_order_id,
    ...
    # transaction
        ...
        unsupported_transaction_types = [EXEC_TRANS_TYPE.CANCEL,
                EXEC_TRANS_TYPE.CORRECT]  # EXEC_TRANS_TYPE.NEW, EXEC_TRANS_TYPE.STATUS
    ...
    # ###############################################
        ...
        terminal_states = [ORDER_STATUS.FILL, ORDER_STATUS.EXPIRED,
    ...
    def _handle_cancel_reject(self, cr):
        ...
        logger.warning('Cancel reject: order_id = %s, orig_id = %s, reason =%s'
                       , order_id, orig_id, cr.cancel_reject_reason)
    ...
    #  (venue, symbol, side, price, qty)
        ...
        logger.info(
            'send_new_order: venue = %s, symbol = %s, side = %s, price = %f, size = %s'
                ,
            venue,
            symbol,
            side,
    ...
    # print "Attempting to cancel/replace %s to price=%s qty=%s" % (order_id, price, qty)
        ...
        logger.info('send_cancel_replace: %s to price=%s qty=%s'
                    % (order_id, price, qty))
    ...
    # print "Attempting to cancel order %s" % order_id
        ...
        assert order_id in self.orders, 'Unknown order %s' \
            % str(order_id)
        assert order_id in self.live_order_ids, \
            "Can't cancel dead order %s" % str(order_id)
    ...
    # return self.send_cancel_replace(order.id, price = price, qty = qty)
        ...
        return self.send_synth_cancel_replace(order.id, price=price,
                qty=qty)
