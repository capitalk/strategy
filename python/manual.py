...
def print_market_data():
    ...
    # raise RuntimeError(str( bid_entry) + "/" + str( offer_entry))
        ...
        if bid_entry:
            bid_str = '%d @ %s (%s)' % (bid_entry.size,
                    bid_entry.price, bid_entry.venue)
        else:
            bid_str = '<none>'
        if offer_entry:
            offer_str = '%d @ %s (%s)' % (offer_entry.size,
                    offer_entry.price, offer_entry.venue)
        else:
            offer_str = '<none>'
        md_window.addstr(4 + i, 5, '%s : bid = %s, offer = %s' % (sym,
                         bid_str, offer_str))
    md_window.refresh()
...
def print_live_orders():
    ...
    for (i, order_id) in enumerate(order_manager.live_order_ids):
        ...
        displayed_id = i + 1
        msg = \
            '%d) venue = %d, symbol = %s, side = %s, price = %s, size = %s' \
            % (
            displayed_id,
            order.venue,
            order.symbol,
