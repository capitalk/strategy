...
class Position:
    ...
    def __str__(self):
        return "Position<symbol=%s long_pos=%f, long_val=%f, long_avg_price=%f short_pos=%f, short_val=%f, short_avg_price=%f net_pos=%f>" % (self.symbol, self.long_pos, self.long_val, self.long_avg_price(),  self.short_pos, self.short_val, self.short_avg_price(), self.net_pos())
...
class Order:
 ...
 #   self.last_update_time = time.time()
  ...
  def __str__(self):
      ...
      (str(self.id), ORDER_STATUS.to_str(self.status), self.venue, self.side, self.price, self.qty, self.symbol, self.cum_qty, self.leaves_qty, self.last_price, self.last_shares, self.last_update_time) 
...
class OrderManager:
  ...
  def _handle_execution_report(self, er):
    ...
    logger.info("Execution: venue_id = %d, cl_order_id = %s, orig_cl_order_id = %s, status = %s, exec_type = %s, transaction_type= %s, side = %s, price = %f, qty = %d, cum_qty = %d, leaves_qty = %d, avg_price = %f, last_shares= %f, last_price = %f", 
      venue_id, cl_order_id, orig_cl_order_id, EXEC_TYPE.to_str(status), EXEC_TYPE.to_str(exec_type), transaction_type, side, price, qty, cum_qty, leaves_qty, avg_price, last_shares, last_price)     
    ...
    if transaction_type == EXEC_TRANS_TYPE.NEW:
        if exec_type == EXEC_TYPE.NEW:
            assert self.pending.has_value(cl_order_id), \
                "Received new order for unknown ID <orig_cl_order_id=%s, cl_order_id=%s>" %  (orig_cl_order_id, cl_order_id)
            ...
            if not self.is_pending(cl_order_id):
                logger.warning("Unknown cancel for <orig_cl_order_id=%s, cl_order_id=%s> - NOT IN PENDING", str(orig_cl_order_id), str(cl_order_id))
            else:
                assert orig_cl_order_id in self.live_order_ids, "Order: %s cancelled not in live orders" % (orig_cl_order_id)   
                ...
                assert self.pending.has_value(cl_order_id),  "Rejected order not in pending <orig_cl_order_id=%s  cl_order_id=%s" % (orig_cl_order_id, cl_order_id)
                self.pending.remove_value(cl_order_id)
                ...
                logger.info("RECEIVED PENDING CANCEL <orig_cl_order_id=%s, cl_order_id=%s>", str(orig_cl_order_id), str(cl_order_id))

        elif exec_type == EXEC_TYPE.REPLACE:
            if not self.is_pending(cl_order_id):
                logger.warning("Replace not in pending <orig_cl_order_id=%s, cl_order_id=%s>", str(orig_cl_order_id), str(cl_order_id)) 
  ...
  def _handle_cancel_reject(self, cr):
    ...
    logger.warning("Cancel reject: cl_order_id = %s, orig_cl_order_id = %s, reason =%s", 
      cl_order_id, orig_cl_order_id, cr.cancel_reject_reason)
    assert orig_cl_order_id in self.orders, \
      "Cancel reject for unknown original order ID %s" % str(orig_cl_order_id)
  ...
  def send_new_order(self, venue, symbol, side, price, qty, order_type = LIM, time_in_force = GFD):
    ...
    order_id = fresh_id()
    logger.info("send_new_order: id=%s, venue=%s, symbol=%s, side=%s, price=%f, size=%s" , order_id, venue, symbol, side, price, qty)
  ...
  def send_cancel_replace(self, order_id, price, qty):
    ...
    request_id = fresh_id()
    logger.info("send_cancel_replace: orig_id=%s, replace_id=%s price=%s qty=%s" % (order_id, request_id, price, qty))
